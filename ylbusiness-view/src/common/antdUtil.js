 ...
 */
export function openNotice(type = 'info', desc = '', title = '提示', duration = 3, component = 'td') {
  if (component === 'td') {
      tdnotice.open(type === 'warn' ? 'warning' : type, desc, title, duration);
    } else {
      notification[type === 'warn' ? 'warning' : type]({
          message: title,
          description: desc,
          duration,
        });
 ...
 * @returns {*}
 */
export function buildFixedLength(text, length) {
  let vdom = null;
  const style = length ? { width: parseInt(length, 10) - 8 } : {};
  vdom = (
