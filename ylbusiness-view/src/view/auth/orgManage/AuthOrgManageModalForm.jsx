...
class AuthAgtAddModalForm extends React.Component {
   ...
   render() {
       ...
       //支付宝费率校验
       const aliProps = getFieldProps(idObj.aliRate ? idObj.aliRate : "aliRate", {
         initialValue: formData.aliRate,
         validate: [{
           rules: [
             { pattern: /^[0-9.]*$/g, message:'支付宝费率只能是数字和"."的组合' }
           ],
           trigger: 'onBlur',
         }]
       });
       //微信费率校验
       const wxProps = getFieldProps(idObj.wxRate ? idObj.wxRate : "wxRate", {
         initialValue: formData.wxRate,
         validate: [{
           rules: [
             { pattern: /^[0-9.]*$/g, message:'微信费率只能是数字和"."的组合' }
           ],
           trigger: 'onBlur',
         }]
       });
       //回调地址校验
       const returnProps = getFieldProps(idObj.returnUrl ? idObj.returnUrl : "returnUrl", {
         initialValue: formData.returnUrl,
         validate: [{
           rules: [
             { pattern: /^(http|https):\/\//, message:'回调地址必须以http(s)://开头' }
           ],
           trigger: 'onBlur',
         }]
       });
        ...
        return (
            <Form horizontal form={this.props.form}>
                ...
                <Row>
                    <Col sm={24} md={24}>
                        <FormItem
                            label="支付宝费率：" labelCol={{span: 6}} wrapperCol={{span: 14}}>
                            <Input {...aliProps} maxLength="20" placeholder="请输入支付宝费率"/>
                        </FormItem>
                    </Col>
                </Row>
                <Row>
                    <Col sm={24} md={24}>
                        <FormItem
                            label="微信费率：" labelCol={{span: 6}} wrapperCol={{span: 14}}>
                            <Input {...wxProps} maxLength="20" placeholder="请输入微信费率"/>
                        </FormItem>
                    </Col>
                </Row>
                <Row>
                    <Col sm={24} md={24}>
                        <FormItem
                            label="回调地址：" labelCol={{span: 6}} wrapperCol={{span: 14}}>
                            <Input {...returnProps} maxLength="20" placeholder="请输入回调地址"/>
                        </FormItem>
                    </Col>
                </Row>
            </Form>
