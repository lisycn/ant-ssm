...
import MonthProfitSharReportSearch from './MonthProfitSharReportSearch';
...
class MonthProfitSharReport extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      modalOprType: 0,
  ...
  exportCallback(dat) {
    ...
    const downloadParams = {
      ...
      SAVE_MonthS: pubConstant.PUB_SAVE_MonthS,
      TEMPLATENAME: 'monthProfitSharReport.ftl',   // 模版文件
      SERVICE: 'download.MonthProfitSharManageServiceImpl',  // sysconfig.propertities 配置
      FLAG: 'MER',
    };
  ...
  render() {
    ...
    const tableColumns = [
      ...
      { title: '分润计算日期', width: 120, dataIndex: 'sharDate', render: (text) => buildFixedLength(text, 120) },
      { title: '月交易总额（元）', width: 120, dataIndex: 'payAmt', render: (text) => buildFixedLength(text, 120) },
      ...
      { title: '支付方式', width: 100, dataIndex: 'CHANNELCODE_ZH', render: (text) => buildFixedLength(text, 100) },
    ];
    const toolbar = [
    ...
    return (
      <TdCard hideHead='true' shadow='true'>
        ...
        <p className='br' />
        <TdPageTable
          url={ylagent.agent.queryMonthProfitSharList}
...
export default MonthProfitSharReport;
