...
class MonthProfitSharReportSearch extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      valid: false,
  ...
  handlerDatePickerChange(value, dateString) {
    this.props.form.setFieldsValue({ month: dateString });
  }
      ...
      <Form horizontal className="advanced-search-form">
        ...
        <Row>
          <div hidden='true' style={{ display: 'none' }}>
            <FormItem>
              <Input {...getFieldProps('month') } />
            </FormItem>
          </div>
          <Col sm={24} md={8}>
            <FormItem {...formItemLayout} label="查询月份" >
              <MonthPicker format={monthFormat} onChange={this.handlerDatePickerChange.bind(this)} />
            </FormItem>
          </Col>
        </Row>
        <Row>
...
const noop = () => { };
// 定义组件标签的可配置属性
MonthProfitSharReportSearch.defaultProps = {
  onSubmit: noop,
  onReset: noop,
};

MonthProfitSharReportSearch = Form.create()(MonthProfitSharReportSearch);
export default MonthProfitSharReportSearch;
