...
import { getLoginInfo, requestSelectData, parseDate } from '../../../common/util';
import { ylagent, tdpub } from '../../../config/server';


import AgentSearchForm from './AgentManageSearchForm';
  ...
  */
  ...
  render() {
    ...
    const tableColumns = [
      ...
      { title: '代理商等级', dataIndex: 'agentType', width: 80, render: (text) => buildFixedLength(getTableData('AGENTTYPE', text), 80) },
      { title: '法人姓名', dataIndex: 'legalName', width: 70, render: (text) => buildFixedLength(text, 70) },
      { title: '父代理商名称', dataIndex: 'fathAgentName', width: 100, render: (text) => buildFixedLength(text, 100) },
      { title: '父代理商编号', dataIndex: 'fathAgentId', width: 120, render: (text) => buildFixedLength(text, 120) },
      { title: '保证金', width: 60, dataIndex: 'margin', render: (text) => buildFixedLength(text, 60) },
      { title: '登录状态', width: 90, dataIndex: 'agentStatus', render: (text) => buildFixedLength(getTableData('AGENTSTATUS', text), 90) },
      { title: '交易状态', width: 90, dataIndex: 'frozState', render: (text) => buildFixedLength(getTableData('AGENTFROZSTATE', text), 90) },
      { title: '开户时间', width: 150, dataIndex: 'createDate', render: (text) => buildFixedLength(parseDate(text), 150) },

    ];    
    return (
      <div>
        <TdCard hideHead='true' shadow='true'>
          <AgentSearchForm onSubmit={this.handleSearchFormSubmit.bind(this) }
          ...
          <TdPageTable rowKey={record => record.agentId}
            ...
            scroll={{ x: 1200 }}
            url={ylagent.agent.queryAgentList}
          //  toolbar={toolbar}
...
AgentManage = Form.create()(AgentManage);
