...
import DayProfitSharReportSearch from './DayProfitSharReportSearch';
...
class DayProfitSharReport extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      modalOprType: 0,
  ...
  exportCallback(dat) {
    ...
    const downloadParams = {
      ...
      TEMPLATENAME: 'dayProfitSharReport.ftl',   // 模版文件
      SERVICE: 'download.DayProfitSharManageServiceImpl',  // sysconfig.propertities 配置
      FLAG: 'MER',
    };
  ...
  render() {
    ...
    const tableColumns = [
      ...
      { title: '支付方式', width: 120, dataIndex: 'CHANNELCODE_ZH', render: (text) => buildFixedLength(text, 120) },
    ];
    const toolbar = [
    ...
    return (
      <TdCard hideHead='true' shadow='true'>
        ...
        <p className='br' />
        <TdPageTable
          url={ylagent.agent.querydayProfitSharList}
...
export default DayProfitSharReport;
