...
// 引入商户基本信息管理-查询条件组件
import MerchantInfoSearchForm from './MerchantInfoSearchForm';
// 商户修改费率
import MerchantInfoUpdateRateForm from './MerchantInfoUpdateRateForm';
// 商户绑定代理商
import MerchantInfoBindAgentForm from './MerchantInfoBindAgentForm';
...
class MermManage extends React.Component {
  ...
  handleFormSubmit(d) {
    if (d.BIZ_SALE !== null && d.BIZ_SALE !== '' && d.BIZ_SALE !== undefined) {
      ...
      const opts = {
        url: ylagent.mer.selectUsrNameByUserId,
        type: 'POST',
        dataType: 'json',
        data: param,
      };
  ...
  handlerRecordSts(param) {
    ...
    confirm({
      ...
      onOk() {
        const opt = {
          url: ylagent.mer.updateMerStatus,
  ...
  updMerRatecallbackValid(handleType, errors, data) {
    ...
    if (!!errors) {
      ...
      this.setState({
        ...
        const obj = this;
        const opt = {
          url: ylagent.mer.updateMerRateInfo,
          data,
        };
        callAjax(opt, (result) => {
          if (result.rspCod === rspInfo.RSPCOD_SUCCESS) {
  ...
  render() {
    ...
    return (
      <TdCard hideHead='true' shadow='true'>
        <MerchantInfoSearchForm onSubmit={this.handleFormSubmit.bind(this)} onReset={this.handleFormReset.bind(this)}
        ...
        >
          <Spin spinning={this.state.spinLoading}>
            <MerchantInfoUpdateRateForm formReset={this.state.formReset}
        ...
        >
          <Spin spinning={this.state.spinLoading}>
            <MerchantInfoBindAgentForm formReset={this.state.formReset}
              valid={this.state.modelIsValid}
              oprType={this.state.modalOprType}
              handleType={this.state.handleType}
              formData={this.state.formData}
