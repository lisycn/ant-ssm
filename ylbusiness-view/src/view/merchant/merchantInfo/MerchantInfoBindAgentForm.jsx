...
class MerchantInfoBindAgentForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      valid: false,
  ...
  render() {
    ...
    return (
      <Form horizontal className="compact-form" style={{ margin: 'auto', maxWidth: 400 }}>
        <Row>
          <Input maxLength="15" type="hidden" {...getFieldProps('custId', { initialValue: this.props.formData.custId ? this.props.formData.custId : '' }) } />
          <Col sm={26} md={21}>
            <FormItem label="所属代理商编号：" labelCol={{ span: 8 }} wrapperCol={{ span: 14 }}>
              <Input maxLength="80" {...getFieldProps('agentId', { initialValue: this.props.formData.agentId ? this.props.formData.agentId : '' }) } />
            </FormItem>
          </Col>
        </Row>
        <div>
...
const noop = () => { };
MerchantInfoBindAgentForm.defaultProps = {
...
MerchantInfoBindAgentForm = Form.create()(MerchantInfoBindAgentForm);
export default MerchantInfoBindAgentForm;
